...
const TagBadge = ({ label, color = "#10b981" }) => (
  <span
    ...
    style={{ backgroundColor: color + "0a", color, border: `1px solid ${color}20` }}
  >
...
const SectionCard = ({ title, icon, editLabel = "Edit", children }) => (
  <div className="card-hover bg-white rounded-xl border border-gray-100 overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
    <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
      <div className="flex items-center gap-2.5">
        <span className="text-gray-400">{icon}</span>
        <h3 className="text-[13px] font-semibold uppercase tracking-wide" style={{ color: Brand.theme.colors.text.primary }}>
          {title}
        </h3>
      </div>
      <button
...
const RatePill = ({ label, value }) => (
  <div className="flex flex-col items-center px-5 py-2.5 rounded-lg bg-gray-50/80 border border-gray-100 transition-all hover:bg-gray-100/80 hover:border-gray-200 hover:scale-105 cursor-default">
    ...
    <span className="text-sm font-bold mt-0.5" style={{ color: Brand.theme.colors.text.primary }}>{value}</span>
  </div>
);

const NoteRow = ({ who, text }) => (
  <div className="flex gap-4 py-2.5 border-b border-gray-50 last:border-b-0">
    <span className="text-[11px] font-medium text-gray-400 whitespace-nowrap shrink-0 pt-0.5">{who}</span>
    <span className="text-[13px] leading-relaxed" style={{ color: Brand.theme.colors.text.primary }}>{text}</span>
  </div>
);
...
const Dashboard = () => {
  ...
  if (!rental) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-64px)] px-8" style={{ backgroundColor: "#fafafa" }}>
        <div className="text-center">
          <div
            className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-5"
            style={{ backgroundColor: Brand.theme.colors.danger + "08" }}
          >
            <svg className="w-7 h-7" style={{ color: Brand.theme.colors.danger }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" />
            </svg>
          </div>
          <h2 className="text-lg font-semibold" style={{ color: Brand.theme.colors.text.primary }}>
            Rental Not Found
          </h2>
          <p className="mt-2 text-sm text-gray-500 max-w-xs mx-auto">
            No rental agreement found for <span className="font-mono font-medium text-gray-700">{raNumber}</span>
          </p>
          <button
  ...
  const rentalPeriodStart = rental.rental.checkout.split(" ").slice(0, 3).join(" ");
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] flex flex-col" style={{ backgroundColor: "#f9fafb" }}>
      ...
      <div className="bg-white border-b border-gray-200">
        ...
        <div className="px-6 pt-5 pb-4">
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-xl font-bold" style={{ color: Brand.theme.colors.text.primary }}>
                {rental.customer.fullName}
              </h1>
              <div className="flex items-center gap-2.5 mt-1">
                <span className="text-[13px] font-mono text-gray-500">{rental.raNumber}</span>
                <span className="inline-flex items-center gap-1.5 text-[12px] font-semibold">
                  <span className={`w-2 h-2 rounded-full ${rental.status === "open" ? "pulse-dot" : ""}`} style={{ backgroundColor: rental.status === "open" ? "#4caf50" : "#9ca3af" }} />
                  <span style={{ color: rental.status === "open" ? "#4caf50" : "#9ca3af" }}>
            ...
            <div className="flex items-start gap-8">
              ...
              <div className="text-right">
                <div className="text-[11px] uppercase tracking-wider text-gray-400 font-medium">Current Rental</div>
                {currentStep && (
                  <>
                    <div className="text-sm font-bold mt-0.5" style={{ color: Brand.theme.colors.primary }}>
                      {currentStep.key === "onRental" ? "On Rental" : currentStep.key}
                    </div>
                    <div className="text-[11px] text-gray-500">{currentStep.sub}</div>
                  </>
                )}
              </div>
              {/* Amount Due */}
              <div className="text-right">
                <div className="text-[11px] uppercase tracking-wider text-gray-400 font-medium">Amount Due</div>
                <div className="text-2xl font-bold mt-0.5" style={{ color: Brand.theme.colors.text.primary }}>
                  ${rental.totals.amountDue.toFixed(2)}
                </div>
                <div className="text-[11px] text-gray-400">
                  Total <span className="font-medium">${rental.totals.estTotal.toFixed(2)}</span> · Prepaid <span className="font-medium">${rental.totals.prepayment.toFixed(2)}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="px-6 py-2.5 border-t border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            {rental.customer.preferred && <TagBadge label="Preferred" color="#10b981" />}
            {rental.customer.connectedCar && <TagBadge label="Connected Car" color="#3b82f6" />}
            {rental.customer.company && (
              <TagBadge label={rental.customer.company} color={Brand.theme.colors.primary} />
            )}
          </div>
          <div className="flex items-center gap-5">
            <span className="text-[11px] text-gray-400 font-mono">WIZARD {rental.references.wizard}</span>
            <span className="text-[11px] text-gray-400 font-mono">RES {rental.references.reservation}</span>
            <span className="text-[11px] text-gray-400 font-mono">MVA {rental.references.mva}</span>
          </div>
        </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100">
          <div className="flex items-start divide-x divide-gray-200">
            ...
            <div className="flex-1 pr-6">
              <div className="text-[11px] uppercase tracking-wider text-gray-400 font-medium mb-1">Customer</div>
              <div className="text-[13px] font-semibold" style={{ color: Brand.theme.colors.text.primary }}>
                {rental.customer.fullName}
              </div>
              <div className="text-[12px] text-gray-500 mt-0.5">{rental.customer.phone}</div>
            </div>
            {/* Vehicle */}
            <div className="flex-1 px-6">
              <div className="text-[11px] uppercase tracking-wider text-gray-400 font-medium mb-1">Vehicle</div>
              <div className="text-[13px] font-semibold" style={{ color: Brand.theme.colors.text.primary }}>
                {rental.vehicle.make}
              </div>
              <div className="text-[12px] text-gray-500 mt-0.5">
                {rental.vehicle.year} · {rental.vehicle.colour} · {rental.vehicle.group}
              </div>
            </div>
            {/* Rental Period */}
            <div className="flex-1 px-6">
              <div className="text-[11px] uppercase tracking-wider text-gray-400 font-medium mb-1">Rental Period</div>
              <div className="text-[13px] font-semibold" style={{ color: Brand.theme.colors.text.primary }}>
                {rentalPeriodStart} → {rentalPeriodEnd}
              </div>
              <div className="text-[12px] text-gray-500 mt-0.5">
                {rental.rental.outStation.split("—")[0].trim()} → {rental.rental.inStation.split("—")[0].trim()}
              </div>
            </div>
            {/* Payment */}
            <div className="flex-1 pl-6">
              <div className="text-[11px] uppercase tracking-wider text-gray-400 font-medium mb-1">Payment</div>
              <div className="text-[13px] font-semibold" style={{ color: Brand.theme.colors.text.primary }}>
                ${rental.totals.amountDue.toFixed(2)} due
              </div>
              <div className="text-[12px] text-gray-500 mt-0.5">{rental.payment.card}</div>
            </div>
          </div>
        </div>
              ...
              <div key={step.key} className="flex items-center flex-1 last:flex-initial">
                  ...
                  <span
                    ...
                    style={{
                      color: step.current
                        ...
                        : step.done ? "#374151" : "#9ca3af",
                    }}
                  >
                    {step.key === "checkedOut" ? "Checked Out" : step.key === "onRental" ? "On Rental" : step.key}
                  </span>
                  <span
                    className="text-[10px]"
                    style={{ color: step.current ? Brand.theme.colors.primary : "#9ca3af" }}
                  >
                    {step.sub}
                  </span>
                </div>
                {i < rental.timeline.length - 1 && (
                  <div
                    ...
                    style={{ backgroundColor: step.done ? "#4caf50" : "#e5e7eb" }}
                  />
                )}
              </div>
            ))}
        ...
        <div className="px-6 py-3 border-t border-gray-100 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              style={{ borderColor: Brand.theme.colors.primary + "40", color: Brand.theme.colors.primary }}
            >
              Void
            </button>
          </div>
          ...
          <div className="flex items-center gap-1">
            {tabs.map((tab) => (
              <button
                ...
                style={{
                  color: activeTab === tab.id ? Brand.theme.colors.primary : "#6b7280",
                }}
              >
                {tab.label}
              </button>
      ...
      {/* ═══ TAB CONTENT ═══ */}
      <div className="flex-1 px-6 py-5 pb-20 animate-fade-in" key={activeTab}>
      ...
      {/* ═══ STICKY FOOTER ═══ */}
      <div
        className="fixed bottom-0 left-0 right-0 h-12 flex items-center justify-between px-6 border-t z-40"
        style={{ backgroundColor: "#fff", borderColor: "#e5e7eb" }}
      >
        <div className="flex items-center gap-3">
          <span className="text-[13px] font-mono text-gray-500">{rental.raNumber}</span>
          <span className="w-1 h-1 rounded-full bg-gray-300" />
          <span className="text-[13px] font-medium" style={{ color: Brand.theme.colors.text.primary }}>
            Due <span className="font-bold">${rental.totals.amountDue.toFixed(2)}</span>
          </span>
        </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      >
        <div className="grid grid-cols-2 gap-x-10">
          ...
          <Field label="Address" value={`${rental.customer.address1}, ${rental.customer.cityPost}`} />
        </div>
      </SectionCard>
      ...
      <SectionCard
        ...
        icon={
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10h10zm0 0h6l-2-5h-4" />
          </svg>
        }
      >
        <div className="flex items-center gap-3 mb-5 pb-4 border-b border-gray-50">
          <div className="w-10 h-10 rounded-lg bg-gray-50 flex items-center justify-center">
            <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0zM13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10h10zm0 0h6l-2-5h-4" />
            </svg>
          </div>
          <div>
            <div className="text-[13px] font-semibold" style={{ color: Brand.theme.colors.text.primary }}>
        ...
        <div className="grid grid-cols-2 gap-x-10">
          ...
          <Field label="Mileage Out" value={rental.vehicle.mileageOut?.toLocaleString()} />
  ...
  <div className="max-w-2xl">
    ...
    >
      <div className="grid grid-cols-2 gap-x-10">
        ...
        <Field label="MVA" value={rental.vehicle.mva} />
        <Field label="Mileage Out" value={rental.vehicle.mileageOut?.toLocaleString()} />
        <Field label="Mileage In" value={rental.vehicle.mileageIn?.toLocaleString() || "—"} />
        <Field label="Fuel Out" value={rental.vehicle.fuelOut} />
        <Field label="Fuel Service" value={rental.vehicle.fuelService} />
        <Field label="Damaged" value={rental.vehicle.damaged} />
        <Field label="Accident Reported" value={rental.vehicle.accidentReported} />
      </div>
    </SectionCard>
  </div>
);
  ...
  <div className="max-w-2xl space-y-6">
    ...
    >
      <div className="grid grid-cols-2 gap-x-10">
        ...
        <Field label="Discount" value={rental.rental.discount ? `${rental.rental.discount}%` : "—"} />
      </div>
    </SectionCard>
...
const PaymentTab = ({ rental }) => (
  <div className="max-w-2xl space-y-6">
    ...
    >
      <div className="grid grid-cols-2 gap-x-10">
        <Field label="Estimated Total" value={`$${rental.totals.estTotal.toFixed(2)}`} />
        <Field label="Prepayment" value={`$${rental.totals.prepayment.toFixed(2)}`} />
      </div>
      <div className="mt-5 pt-4 border-t border-gray-100 flex items-center justify-between">
        <span className="text-[13px] font-semibold" style={{ color: Brand.theme.colors.text.primary }}>Amount Due</span>
        <span className="text-xl font-bold" style={{ color: Brand.theme.colors.primary }}>
          ${rental.totals.amountDue.toFixed(2)}
        </span>
      </div>
    </SectionCard>
    ...
    >
      <div className="grid grid-cols-2 gap-x-10">
        ...
        <Field label="Coupon Amount" value={`$${rental.charges.couponAmount}`} />
        ...
        <Field label="Accident Repairs" value={`$${rental.charges.accidentRepairs}`} />
